...
| 4 | OJP version **0.4.21-beta** is used for both the server and the JDBC driver.                                     |
| 5 | The `bench` tool is **built and run on the control node only**. No bench process is deployed on remote machines. |
| 6 | The OJP Server requires **Java 25+** on each proxy node. The control node (bench tool) requires **Java 11+**.    |
...
graph TD
    ...
    subgraph PROXY_TIER ["Proxy Tier — 3 × OJP Server 0.4.21-beta (gRPC :1059)"]
        P1["PROXY-1\nOJP :1059"]
        P2["PROXY-2\nOJP :1059"]
        P3["PROXY-3\nOJP :1059"]
    end
...
# Install Java 25 (OJP Server requires Java 25+)
sudo apt-get update
sudo apt-get install -y wget apt-transport-https gpg
wget -qO - https://packages.adoptium.net/artifactory/api/gpg/key/public \
    | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/adoptium.gpg > /dev/null
...
# Download OJP Server 0.4.21-beta shaded JAR
sudo curl -L \
...
```bash
for PROXY_IP in "${PROXY1_IP}" "${PROXY2_IP}" "${PROXY3_IP}"; do
  ssh ${SSH_USER}@${PROXY_IP} "pkill -f ojp-server-0.4.21-beta-shaded.jar && echo 'OJP stopped on ${PROXY_IP}'"
done
```
