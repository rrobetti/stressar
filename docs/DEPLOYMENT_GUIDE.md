...
| 4 | OJP version **0.4.20-beta** is used for both the server and the JDBC driver.                                     |
...
graph TD
    ...
    subgraph PROXY_TIER ["Proxy Tier — 3 × OJP Server 0.4.20-beta (gRPC :1059)"]
        P1["PROXY-1\nOJP :1059"]
        P2["PROXY-2\nOJP :1059"]
        P3["PROXY-3\nOJP :1059"]
    end
...
# Download OJP Server 0.4.20-beta shaded JAR
sudo curl -L \
...
```bash
for PROXY_IP in "${PROXY1_IP}" "${PROXY2_IP}" "${PROXY3_IP}"; do
  ssh ${SSH_USER}@${PROXY_IP} "pkill -f ojp-server-0.4.20-beta-shaded.jar && echo 'OJP stopped on ${PROXY_IP}'"
done
```
