...
- [Database Configuration](#database-configuration)
  - [SSL/TLS Configuration](#databasessl-object-optional)
...
#### `database.ssl` (object, optional)

SSL/TLS configuration applied to every connection opened by this benchmark run.
When omitted or `enabled: false`, connections use plaintext (no SSL).

See [SSL_ANALYSIS.md](SSL_ANALYSIS.md) for a full analysis of SSL performance impact
and production recommendations.

##### `database.ssl.enabled` (boolean, default: `false`)

Master switch. Set to `true` to enable SSL/TLS.
...
##### `database.ssl.mode` (enum, default: `REQUIRE`)

SSL enforcement and verification level. Maps to the PostgreSQL JDBC `sslmode` parameter.

| Value | Encrypted | Server cert verified | Hostname verified | Recommended for |
|---|---|---|---|---|
| `DISABLE` | No | — | — | Dev / isolated test only |
| `ALLOW` | Opportunistic | No | No | Dev only |
| `PREFER` | Opportunistic | No | No | Dev only |
| `REQUIRE` | Yes | No | No | Basic encryption |
| `VERIFY_CA` | Yes | Yes | No | Internal PKI |
| `VERIFY_FULL` | Yes | Yes | Yes | **Production** |
...
##### `database.ssl.rootCertPath` (string, optional)

Path to the PEM-encoded CA certificate used to verify the server certificate.
Required when `mode` is `VERIFY_CA` or `VERIFY_FULL`.
...
##### `database.ssl.certPath` / `database.ssl.keyPath` (string, optional)

Paths to the PEM-encoded client certificate and private key for **mutual TLS (mTLS)**.
Both must be specified together; leave both unset for server-only authentication.
...
database:
  ...
  ssl:
    ...
    mode: VERIFY_FULL
    rootCertPath: "/etc/ssl/certs/db-ca.pem"
    # certPath: "/etc/ssl/certs/client.pem"  # mTLS only
    # keyPath:  "/etc/ssl/private/client.key" # mTLS only
```

**Note for OJP mode:** the `ssl` block configures the OJP server → PostgreSQL connection
(Leg B). To secure the load-generator → OJP gRPC connection (Leg A) see
[SSL_ANALYSIS.md – OJP Mode](SSL_ANALYSIS.md#ojp-mode-two-leg-setup).
